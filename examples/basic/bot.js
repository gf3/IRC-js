...
bot.observeIf( fmt( " *@?%s *:? *(you(?:['’]?re)?|u(?: r)|ur?) +(.+)", bot.config.nick )
...
bot.observeIf( fmt( "@?%s[: ]+(?:join|add) +([+!#&][^ ]+)(?: +([^ ]+))?", bot.config.nick )
  ...
  bot.channels.add( name, key, function( chan ) {
    msg.reply( fmt( "Ok, I am now in %s%s", name, key ? fmt( ", I used “%s” to get in.", key ) : "." ) )
  })
})
