...
/** Buffer that is reused for all messages passed into `send()' */
const BUFFER_SIZE = MAX_LENGTH + 4;
...
function onData(internal, data) {  
  ...
  for (let i = 0; i < length; ++i) {
    ...
    if (log.level & LEVEL.DEBUG) {
      log.debug("Received: %s", slice.toString(null, start, i - 2));
    }
    let msg = parser.parse(slice);
    msg.client = this;
  ...
  if (remains) {
    internal.set(INTERNAL_BUFFER, buf.slice(length - remains));
  }
}
...
function send(internal, message) {
  message.toBuffer(MESSAGE_BUFFER);
  const length  = MESSAGE_BUFFER.readUInt16LE(0);
  const remains = MESSAGE_BUFFER.readUInt16LE(2);
  write.call(this, internal.get(INTERNAL_SOCKET), MESSAGE_BUFFER.slice(4, length));
