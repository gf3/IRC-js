...
function IRC ( options ) {
  ...
  this._internal.emitter.on( 'ping', function( message ){
    this.raw( CMD.PONG + ' :' + message.params[0] )
  }.bind( this ))
}
...
function parseMessage ( data ) { var buffer, last, message, command, i
  ...
  for ( i = 0; i < buffer.length; i++ ) {
    ...
    if ( command === RPL.WELCOME )
      this._internal.nick = message.params[0]
    else if ( command === CMD.NICK && message.person.nick === this._internal.nick )
      this._internal.nick = message.params[0]

    // Emit event
    this._internal.emitter.emit( EVT.ANY, message.command, message )
...
IRC.prototype.pass = function pass ( password ) {
  ...
  return this.raw( CMD.PASS + param( password ) )
}
...
IRC.prototype.user = function user ( username, wallops, invisible, realname ) { var mode
  ...
  return this.raw( CMD.USER + " " + [ username, mode, '*' ].join(' ') + param( realname, null, ' :' ) )
}
...
IRC.prototype.oper = function oper ( user, password ) {
  // 4.1.5
  return this.raw( CMD.OPER + param( user ) + param( password ) )
}
...
IRC.prototype.quit = function quit ( message ) {
  // 4.1.6
  this.raw( CMD.QUIT + if_exists( message, null, ' :' ) ).disconnect()
  return this
}
...
IRC.prototype.join = function join ( channel, key ) {
  // 4.2.1
  return this.raw( CMD.JOIN + param( channel ) + if_exists( key ) )
}
...
IRC.prototype.part = function part ( channel ) {
  // 4.2.2
  return this.raw( CMD.PART + param( channel ) )
}
...
IRC.prototype.channelMode = function channelMode ( channel, mode, last ) {
  // 4.2.3.1
  return this.raw( CMD.MODE + param( channel ) + param( mode ) + if_exists( last ) )
}
...
IRC.prototype.topic = function topic ( channel, type ) {
  ...
  if ( typeof type === 'function' ) {
    ...
    this.once( RPL.TOPIC, function( message ) {
      type.call( this, message.params[1], message.params[ message.params.length - 1 ] )
    })
    
    this.raw( CMD.TOPIC + param( channel ) )
  }
  else
    this.raw( CMD.TOPIC + param( channel ) + param( type || '', null, ' :' ) )
...
IRC.prototype.list = function list ( channel, hollaback ) {
  ...
  if ( not_blank( channel ) ) {
    ...
    this.once( RPL.LIST, function( message ) {
      hollaback.call( this, [message.params[1], message.params[2], message.params[3]] )
    })
  }
  
  this.raw( CMD.LIST + if_exists( channel ) )
  return this
}
...
function gatherList () {
  ...
  this._internal.emitter.on( RPL.LIST, gatherChannels )
  this._internal.emitter.on( RPL.LISTEND, function() {
    this._internal.emitter.removeListener( RPL.LIST, gatherChannels )
...
IRC.prototype.invite = function invite ( nickname, channel ) {
  // 4.2.7
  return this.raw( CMD.INVITE + param( nickname ) + param( channel ) )
}
...
IRC.prototype.kick = function kick ( channel, user, comment ) {
  // 4.2.8
  return this.raw( CMD.KICK + param( channel ) + param( user ) + if_exists( comment, null, ' :' ) )
}
...
IRC.prototype.version = function version ( server, hollaback ) {
  ...
  return this
    .once( RPL.VERSION, function( message ) {
      hollaback.call( this, message.params.slice(1) )
    })
    .raw( CMD.VERSION + if_exists( server ) )
}
...
function sendPrivmsg ( receiver, msg ) {
  this.raw( CMD.PRIVMSG + param( receiver ) + ' ' + param( msg || '', null, ':' ) )
}
...
IRC.prototype.notice = function( receiver, msg ) {
  ...
  this.raw( CMD.NOTICE + param( receiver ) + ' ' + param( msg || '', null, ':' ) )
  return this
}
