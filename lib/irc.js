 ...
 * IRC#whois ( nick ) -> self
 *
 * Whois a persion. Will emit a 'whois' event with the first argument being an
 * array of parsed messages like other events usually have
 **/
IRC.prototype.whois = function( nick ) {
  var queue = this._internal.whois_queue[nick]
  if ( queue ) { // WHOIS in progress for that nick, ignore it
    return
  }
  else {
    queue = this._internal.whois_queue[ nick ] = []
  }
  
  function whois_handler( command, message ) {
    if (/3\d\d/.test( command ) ) {
      queue.push( message )
    }
  }
  this._internal.emitter.on( '*', whois_handler );
  this._internal.emitter.once( '318', function remove_whoishandler() {
    this._internal.emitter.removeListener( '*', whois_handler )
    this._internal.emitter.emit( 'whois', queue )
    delete this._internal.whois_queue[ nick ]
  }.bind( this ) )
  return this.raw('WHOIS' + param( nick ) )
}
...
**/
IRC.info = JSON.parse( fs.readFileSync( path.join( __dirname, '..', 'package.json' ), 'utf8' ) )
