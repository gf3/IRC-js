...
IRC.prototype.connect = function connect ( hollaback ) {
  // Client setup
  var not_open
  if ( !internal.stream || ( not_open = ( [ 'open', 'opening' ].indexOf( internal.stream.readyState ) < 0 ) ) ) {
    if ( not_open ) {
      internal.stream.end()
      internal.stream.removeAllListeners()
      internal.stream = null
    }

    if ( this.options.ssl )
      internal.stream = require( 'tls' ).connect( this.options.port, this.options.server )
    else
      internal.stream = new require( 'net' ).Stream()

    internal.stream.setEncoding( this.options.encoding )
    internal.stream.setTimeout( 0 )

    // Forward network errors
    internal.stream.on( 'error', function( er ) {
      internal.emitter.emit( 'error', er )
      internal.emitter.emit( 'error:network', er )
    })

    // Receive data
    internal.stream.on( 'data', parseMessage.bind( this ) )

    // Timeout
    internal.stream.on( 'timeout', do_disconnect.bind( this ) )

    // End
    internal.stream.on( 'end', do_disconnect.bind( this ) )

    if ( !this.options.ssl )
      internal.stream.connect( this.options.port, this.options.server )
  }
...
IRC.prototype.raw = function raw ( data ) {
  if ( internal.stream.readyState == 'open' ) {
    data = data.slice( 0, 509 )
    if ( !/\r\n$/.test( data ) )
      data += "\r\n"
    internal.stream.write( data )
    if ( this.options.log )
