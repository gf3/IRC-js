...
const disconnect = function( internal ) {
  ...
  if ( since )
    info.call( this, "[INFO]  Connected at %s, disconnected at %s", since, new Date() )
...
const onData = function( internal, data ) {
  ...
  for ( i = 0; i < buffer.length; ++i ) {
    ...
    if ( null === message ) {
      error.call( this, "[ERROR] Failed parsing %s", buffer[i] )
      continue
    }
    // Give superpowers
    message.with( this )
...
const connect = function( internal, callback ) {
  ...
  if ( internal.connected ) {
    warn.call( this, "[WARN]  Already connected")
    return this
  }
  ...
  if ( 2 === arguments.length )
    this.addListener( EVENT.CONNECT, callback.bind( this ) )
...
const write = function( internal, data ) {
  if ( internal.socket.readyState !== "open" ) {
    error.call( this, "Socket is not open, but tried to send: %s", data )
    return this
  }
  const crlf = /\r\n$/.test( data ) ? "" : "\r\n"
...
const addListener = function( internal, event, callback ) {
  internal.emitter.addListener( event, callback.bind( this ) )
  return this
}
 ...
 *  @return {IRC}
 */
IRC.prototype.listenFor = function( predicate, listener, command ) {
  const isFunc = predicate instanceof Function
      , regExp = isFunc ? null : RegExp( predicate ) 
      , predic = isFunc ? predicate : matchMessage.bind( this, regExp )
  return listenFor.call( this, predic, listener, command || COMMAND.PRIVMSG )
}
...
const listenFor = function( pred, then, command ) {
...
// Is there any good standard stuff for this?
...
exports.IRC   = IRC
exports.cache = { people: ircmap.cache.people }
