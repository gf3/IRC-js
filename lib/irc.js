...
// Keys for internals
const INTERNAL_MESSAGE_BUFFER = 0;
const INTERNAL_STREAM_BUFFER  = 1;
const INTERNAL_CONNECTED      = 2;
const INTERNAL_SIGNALS        = 3;
const INTERNAL_SOCKET         = 4;
...
function Client(conf) {
  ...
  internal.set(INTERNAL_MESSAGE_BUFFER, new Buffer(MESSAGE_BUFFER_SIZE));
  internal.set(INTERNAL_STREAM_BUFFER, null);
...
function onData(internal, data) {
  const prev = internal.get(INTERNAL_STREAM_BUFFER);
  if (prev) {
  ...
  if (end !== length) {
    internal.set(INTERNAL_STREAM_BUFFER, data.slice(end));
  }
}
...
function send(internal, message) {
  const buffer = internal.get(INTERNAL_MESSAGE_BUFFER);
  message.toBuffer(buffer);
  const length  = buffer.readUInt16LE(0);
  const remains = buffer.readUInt16LE(2);
  write.call(this, internal.get(INTERNAL_SOCKET), buffer.slice(4, length));
