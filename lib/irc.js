...
function IRC( options ) {
  ...
  **/
  this.list = function( channel, hollaback ) {
    if (!this._listQueue) { //at first call create queue object
      this._listQueue = async.queue(function(task, callback){ //getting rid of of locks
        if ( !task.hollaback )
          task.hollaback = task.channel, task.channel = null

        gatherList(function(list) {
          task.hollaback.call( this, list );
          callback();//this queue item is finished asking for next one
        });

        this.raw( 'LIST' + if_exists( task.channel ) )
      }.bind(this), 1); //only one list command could be in queue
    }
    
    this._listQueue.push({channel: channel, hollaback: hollaback});
    
    return this
  }
  
  function gatherList (callback) {
    internal.cache.channel_list = []
    
    var gatherChannels = function( message ) {
      internal.cache.channel_list.push( [message.params[1], message.params[2], message.params[3]] )
    }
    
    var processListFinish = function() {
      emitter.removeListener( '322', gatherChannels );
      emitter.removeListener( '323', processListFinish );
      callback(internal.cache.channel_list);
      internal.cache.channel_list = []
    }
    
    emitter.on( '322', gatherChannels )
    emitter.on( '323', processListFinish )
  }
