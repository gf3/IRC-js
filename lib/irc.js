...
const disconnect = function( internal ) {
  ...
  if ( since )
    logger.info( "Connected at %s, disconnected at %s", since, new Date() )
  return this
}
...
const onData = function( internal, data ) {
  ...
  for ( i = 0, l = buffer.length; i < l; ++i ) {
    logger.debug( "Received: %s", buffer[i] )

    message = parser.message( buffer[i] + DELIM, this.config.die )

    if ( null === message ) {
      logger.error( "Failed parsing %s", buffer[i] )
      continue
    }
    // Give superpowers
    message.for( this )
...
const connect = function( internal, callback ) {
  ...
  if ( internal.connected ) {
    logger.warn( "Already connected at %s", internal.connectedSince )
    return this
  }
...
const write = function( sock, data ) {
  if ( sock.readyState !== NODE.SOCKET.STATE.OPEN ) {
    logger.error( "Socket is not open, but tried to send: %s", data )
    return this
  }
