...
function IRC(options) {
  ...
  // Client setup
  var stream = net.createConnection(this.options.port, this.options.server);
  stream.end(); // XXX It's shitty that I have to do this, but it connects immediately
  ...
  stream.addListener("connect", function () {
    ...
    while (notConnected) notConnected = !(stream.readyState == "open");
    internal.connected_since = Date.now();
  }.bind(this));
  
  function on_connect(context) {
    internal.connected = true;
    if (! typeof context.options.pass === "undefined") stream.pass(context.options.pass);
    context.nick(context.options.nick);
    context.user(context.options.user.username, context.options.user.hostname, context.options.user.servername, context.options.user.realname);
    return true;
  }
  ...
  function parseMessage(data) {
    ...
    try {
      for (var i = 0; i < buffer.length; i++) {
        ...
        if (!internal.connected) on_connect(this) && emitter.emit("connected");
  ...
  function buildMessage(node) {
    ...
    switch (node.type) {
      ...
      case "person":
        var p = {};
        for (i=0; i<node.children.length; i++) p[node.children[i].type] = node.children[i].value.toString();
        return ["person", p];
