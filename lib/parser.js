...
const nick      = rule( "[`_", common, cbracket, cflex, rsolid, "]+" )
    ...
    , params    = rule( "((?:", middle, space, "?)*", space, "*(?:", trailing, ")?)" )
...
/** Parse a mode string, e.g. "+im-t", into arrays of chars.
 *  using value from the flags param
 *  @param {string} data
 *  @return {Array.<boolean|string>}
 */
const parseMode = function( data ) {
  ...
  var l = match.length
    ...
    , j
  while ( l-- ) {
    sign = match[ l ][ 0 ] // '+' means set, and '-' unset.
    for ( i = 1, j = match[ l ].length; i < j; ++i ) {
      flag = match[ l ][ i ]
        modes.push( sign === '+', flag )
    }
  }
  return modes // [true, m, false, i, ...]
}
