...
const onJoinCommand = function( msg ) {
  ...
  logger.debug( "Adding user %s to channel %s", prsn, name )
    ...
    if ( chan = this.channels.get( objects.id( chans[l] ) ) )
      for ( i = 0; i < k; ++i )
        if ( users[i] === this.user.nick ) {
          ...
          logger.debug( "I was kicked from %s, removing it", chan.name )
          this.channels.delete( chan.id )
        } else {
          logger.debug( "%s was kicked from %s, removing them", users[i], chan.name )
          chan.people.delete( objects.id( users[i] ) )
        }
  }
  return STATUS.SUCCESS
...
const onModeCommand = function( msg ) {
  ...
  if ( ! target ) {
    logger.warn( "Got mode %s for %s, dunno what to do", msg.params[1], param )
    return STATUS.ERROR
  }
  if ( this.user === target )
    logger.debug( "Setting mode %s for myself", msg.params[1] )
  else
    logger.debug( "Setting mode %s for %s", msg.params[1], target )
...
const onPartCommand = function( msg ) {
  ...
  if ( chan && chan.people.has( objects.id( nick ) ) ) {
    ...
    logger.debug( "Removing %s from %s", nick, chan )

    if ( nick === this.user.nick ) {
      logger.debug( "Left %s, removing it", chan )
  ...
  if ( chan ) {
    logger.error( "Got a part message from %s for channel %s, but %s was not in that channel"
                , nick, name, nick )
    return STATUS.ERROR
  }

  logger.error( "Got a part message from %s for channel %s, which I am not in"
              , nick, name )
  return STATUS.ERROR
}
...
const onTopicCommand = function( msg ) {
  ...
  if ( chan ) {
    if ( chan.topic )
      logger.debug( "Updating topic for %s from %s to %s"
                  , chan, chan.topic, topic )
    else
      logger.debug( "Setting topic for %s to %s", chan, topic )
    chan.topic = topic
    return STATUS.SUCCESS
  }
  logger.warn( "Got a topic (%s) for channel %s, which I am not in"
            , topic, msg.params[0] )
...
const onQuitCommand = function( msg ) {
  ...
  logger.debug( "Got a quit message for %s, removing them from all channels", user )
  return STATUS.SUCCESS
}
...
const onMyInfoReply = function( msg ) {
  ...
  logger.debug( "Updating server name from %s to %s", this.server.name, name )
...
const onNameReply = function( msg ) {
  const chan  = this.channels.get( objects.id( msg.params[2] ) )
      ...
      , count = nicks.length
  if ( ! chan ) {
    logger.error( "Got a name reply for unknown channel %s", msg.params[2] )
    return STATUS.ERROR
  }
  ...
  for ( ; i < count; ++i ) {
    ...
    logger.debug( "Adding user %s to channel %s", prsn.nick, chan )
  }
  return STATUS.SUCCESS
}
...
const onWelcomeReply = function( msg ) {
  ...
  logger.debug( "Setting nick to", nick )
  this.user.nick = nick
  return STATUS.SUCCESS | STATUS.REMOVE
}
...
const onAnyError = function( msg ) {
  ...
  logger.error( "Received error %s from %s with params %s"
...
const onForwardError = function( msg ) {
  ...
  if ( this.channels.has( objects.id( to ) ) ) {
    logger.debug( "Forwarded from %s to %s, which already existed", from, to )
    return
  }
  ...
  logger.info( "Got forwarded from %s to %s, adding %s", from, to, to )
