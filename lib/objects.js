...
const reply = function( irc, text ) {
  const sender = this.params[0]
      , recipient = sender === irc.config.nick
                  ? this.prefix : sender
...
const invite = function( irc, subject ) {
  const isChan = this instanceof Channel
      ...
      , user   = isChan ? ( subject instanceof Person
                        ...
                        : this.nick
  irc.send( message( COMMAND.INVITE, [ chan, user ] ) )
  return this
}
...
const getVersion = function( irc, server ) {
  irc.send( message( COMMAND.VERSION, [ server ] ) )
  return this
}

const notify = function( irc, notification ) {
  irc.send( message( COMMAND.NOTICE
          , [ this, trailing( notification ) ] ) )
  return this
}
